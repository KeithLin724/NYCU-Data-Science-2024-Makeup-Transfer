...
class MAKEUP(Dataset):
    def __init__(self, image_path, transform, mode, transform_mask, cls_list):
        ...
        self.A_seg = glob.glob(os.path.join(image_path, MT_MODE, "non-makeup", "*.png"))
        self.As = [
            os.path.join(image_path, "images", "non-makeup", x.split("/")[-1])
            for x in self.A_seg
        ]
        self.B_seg = glob.glob(os.path.join(image_path, MT_MODE, "makeup", "*.png"))
        self.Bs = [
            os.path.join(image_path, "images", "makeup", x.split("/")[-1])
            for x in self.B_seg
        ]
    ...
    def __getitem__(self, index):
        if self.mode == "train":
            ...
            return (
                ...
                self.transform_mask(mask_A),
                self.transform_mask(mask_B),
            )
